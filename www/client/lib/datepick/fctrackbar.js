...
fctrackbar.hotSearch = function(id) { // Constructor
	// Vars
	this.id = id;
...
fctrackbar.hotSearch.prototype = {
// Const
	ERRORS : {
...
// Methods
	...
	*/
	...
	init : function(hash, node) {
		...
		try {
			...
			var code = '<div class="tickdiv" id="'+this.TICKDIV_PREFIX+this.id+'"></div><table id="'+this.TABLE_PREFIX+this.id+'" ' + (this.width ? ' style="width:'+this.width+'px;"' : '') + 'class="trackbar" onSelectStart="return false;">\
				<tr>\
					<td class="l"><div id="leftBlock_' + this.id + '"><span></span><span class="limit"></span><img class="fcTrackBar_left" id="leftBegun_' + this.id + '" ondragstart="return false;" src="/i/0.gif" /></div></td>\
					<td class="c" id="centerBlock_' + this.id + '"></td>\
					<td class="r"><div id="rightBlock_' + this.id + '"><span></span><span class="limit"></span><img class="fcTrackBar_right" id="rightBegun_' + this.id + '" ondragstart="return false;" src="/i/0.gif" /></div></td>\
				</tr>\
			</table>';
			...
			// Set limits
			if (!this.clearLimits) {
				this.leftBlock.firstChild.nextSibling.innerHTML = Math.floor(this.leftLimit/60)+((this.leftLimit%60>9)?':':':0')+this.leftLimit%60;
				this.rightBlock.firstChild.nextSibling.innerHTML = Math.floor(this.rightLimit/60)+((this.rightLimit%60>9)?':':':0')+this.rightLimit%60;
			}
			// Do it!
	...
	drawTicks : function() {
		if (this.showBigTicks) for (var i = 1; i <= this.bigTicks; i++) this.tickDiv.innerHTML += '<div style="left:'+(this.offset+this.widthRem + i * this.valueWidth / (this.bigTicks + 1))+'px" class="tick"><span>' + Math.round(parseInt(this.leftLimit + i * this.valueInterval/ (this.bigTicks + 1))/60)+ '</span></div>';
		if (this.showSmallTicks) for (i = 1; i <= (this.smallTicks + 1) * (this.bigTicks + 1) - 1; i++) this.tickDiv.innerHTML += '<div style="left:'+(this.offset+this.widthRem + i * this.valueWidth / ((this.smallTicks + 1) * (this.bigTicks + 1)))+'px" class="stick"></div>';
	},
	...
	setCurrentState : function() {
		...
		if (!this.clearValues) this.leftBlock.firstChild.innerHTML = (!this.dual && this.leftWidth > this.width / 2) ? "" : Math.floor(this.leftValue/60)+((this.leftValue%60>9)?':':':0')+this.leftValue%60;
		if(!this.dual) {
		...
		if (!this.clearValues) this.rightBlock.firstChild.innerHTML = (!this.dual && this.rightWidth >= this.width / 2) ? "" : Math.floor(this.rightValue/60)+((this.rightValue%60>9)?':':':0')+this.rightValue%60;
		if(!this.dual) {
